...
public class ItemService {
        ...
        Stream<Item> filter = allData
            ...
            .filter(
                item -> {
                    if (item.getDescription() != null) {
                        return (
                            item.getTitle().toLowerCase().contains(data.toLowerCase()) ||
    ...
    public List<Item> filterByLocation(Long lat, Long lon, Double distance) {
        List<Item> allData = itemRepository.findAll();
        Stream<Item> filter = allData.stream().filter(item -> distance(item.getLat(), item.getLon(), lat, lon) < distance);
